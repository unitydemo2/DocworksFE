...
The _Navigation Areas_ define how difficult it is to walk across a specific area, the lower cost areas will be preferred during path finding. In addition each NavMesh Agent has an _Area Mask_ which can be used to specify on which areas the agent can move.
...
![Nodes and links visited during pathfinding.](../uploads/Main/NavMeshNodePositions.svg)
...
- [NavMeshAgent.areaMask](ScriptRef:AI.NavMeshAgent-areaMask) - Script API to set areaMask for an agent.
- [NavMeshAgent.SetAreaCost()](ScriptRef:AI.NavMeshAgent.SetAreaCost.html) - Script API to set area cost for an agent. 
